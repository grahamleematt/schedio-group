...
- All work lives on `idea/<firstname>/<slug>` branches. The `/idea` command
...
- When they are happy — or the session is wrapping up — run `/share` so the
  work becomes a draft PR with screenshots and a Slack post. Unshared work is
  invisible to Matthew.
- If things get confusing or broken, offer `/reset` to return to a clean,
  up-to-date state. Their data is a sandbox; resetting is always safe.
...
- `/idea <what to try>` — start a new idea branch and dev server.
- `/share` — screenshots + draft PR + Slack post to #sg-dream.
- `/try` — check out the other person's idea branch to react to it.
- `/suggest <idea>` — no code; files a GitHub issue and posts to Slack.
